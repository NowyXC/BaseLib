...
public class StatusBarCompat {
    ...
    static {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            IMPL = new IStatusBar() {
                @Override
                public void setStatusBarColor(Window window, @ColorInt int color,boolean fitSystemWindows) {
                }
            };
        }
    }
    ...
    public static void setStatusBarColor(Activity activity, @ColorInt int color, boolean lightStatusBar) {
        setStatusBarColor(activity.getWindow(), color, lightStatusBar,true);
    }


    public static void setStatusBarColor(Activity activity, @ColorInt int color, boolean lightStatusBar,boolean fitSystemWindows) {
        setStatusBarColor(activity.getWindow(), color, lightStatusBar,fitSystemWindows);
    }
     ...
     * @param lightStatusBar if the status bar color is light. Only effective in some devices.
     */
    public static void setStatusBarColor(Window window, @ColorInt int color, boolean lightStatusBar,boolean fitSystemWindows) {
        if ((window.getAttributes().flags & WindowManager.LayoutParams.FLAG_FULLSCREEN) > 0
                || StatusBarExclude.exclude) {
            return;
        }
        IMPL.setStatusBarColor(window, color,fitSystemWindows);
